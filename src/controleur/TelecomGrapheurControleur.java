...
public class TelecomGrapheurControleur implements MouseListener, MouseMotionListener, MouseWheelListener, ActionListener {
    ...
    public void mousePressed(MouseEvent e) {
        if (SwingUtilities.isLeftMouseButton(e)) { // si c'est un clic gauche
            ...
            for (PointModele p : this.modele.getListesPoints().getListePoints()) { // On parcourt la liste de point
                if (e.getX() == p.getX() && e.getY() == ((int) (p.getY() + 1000000055))) { // On verifie si on a deja clic � cet endroit
                    this.modele.getListesPoints().getListePoints().remove(p); // si le point est trouv�, on l'enleve 
            ...
            if (!exist) { // si aucun point n'a ete trouve
                this.modele.getListesPoints().getListePoints().add(new PointModele(e.getX(), e.getY() - 1000000055)); // On set un point � un endroit invisible
            }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (this.modele.isClique()) { // si on a fait un clic gauche
            ...
            if (this.decalageXOrigin > 0) { // si on a dragg de gauche vers droite
                this.modele.getBornes().setBorneXLeft(this.modele.getBornes().getBorneXLeft() - this.decalageXOrigin); // On met � jour les bornes 
            } else { // si c'est l'inerse
                this.modele.getBornes().setBorneXRight(this.modele.getBornes().getBorneXRight() - this.decalageXOrigin); // On met � jour les bornes
            }
            this.modele.setClique(false); // Maintenant qu'on a calcul� le declalage, on ne veut plus repasser dedans jusqu'au prochain clic
        }
